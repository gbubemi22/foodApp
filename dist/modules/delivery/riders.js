...
export const findNearbyRiders = async (vendorId, orderId) => {
    ...
    await Promise.all(nearbyRiders.map(async (rider) => {
...
export const acceptOrder = async (riderId, orderId) => {
    const order = await Order.findById(orderId);
    ...
    const updatedOrder = await Order.findByIdAndUpdate(orderId, { riderId: riderId }, { new: true });
    if (!updatedOrder) {
...
export const updateRiderLocation = async (riderId, newLocation) => {
    ...
    const updatedLocation = await Track.findOneAndUpdate({ riderId }, {
        $set: {
            "currentLocation.latitude": newLocation.latitude,
            "currentLocation.longitude": newLocation.longitude,
        },
    }, { new: true });
    await Rider.findOneAndUpdate({ _id: riderId }, {
        $set: {
    ...
    if (!updatedLocation) {
        throw new NotFoundError(`Failed to update rider location for rider ${riderId}`);
    }
    return updatedLocation;
};
export const getRiderCurrentLocationByOrder = async (orderId) => {
