...
import { StatusEnum } from "./type.js";
...
export const create = async (payload) => {
    ...
    return {
        status: true,
        message: "Your account is awaiting verification, you will be notified about your status soon.",
        data: [],
    };
};
export const login = async (email, password) => {
    ...
    if (vendor.status === StatusEnum.PENDING ||
        vendor.status === StatusEnum.REJECTED) {
        throw new BadRequestError(`Please contact admin`);
    }
    if (vendor.block === true) {
        throw new BadRequestError(`Your account has been blocked please contact admin`);
    }
...
export const changePassword = async (vendorId, currentPassword, newPassword) => {
    ...
    await Vendor.findOneAndUpdate({ _id: vendorId }, { $set: { password: hashedPassword } }, { new: true, runValidators: true });
    return {
        success: true,
