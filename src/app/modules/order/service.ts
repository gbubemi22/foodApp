...
export const create = async (userId: string, payload: OrderData) => {
  ...
  for (const payloadItem of payload.items) {
    ...
    const finalPrice =
      dbItem.price - discount > 0 ? dbItem.price - discount : 0;
    ...
    for (const payloadExtra of payload.extras) {
      ...
      if (!dbExtra)
        throw new NotFoundError(
          `Extra item with ID ${payloadExtra.itemId} not found`
        );
...
export const trackOrder = async (trackId: string, userId: string) => {
...
export const listOneVendorOrder = async (orderId: string, vendorId: string) => {
...
export const listVendorsOrders = async (vendorId: string) => {
